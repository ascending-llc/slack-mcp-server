...
| `SLACK_MCP_REQUEST_TIMEOUT`       | No        | `30s`                     | HTTP request timeout for Slack API calls. Use Go duration format (e.g., `60s`, `2m`, `500ms`)                                                                                                                                                                                             |
...
| `SLACK_MCP_CLIENT_CACHE_SIZE`     | No        | `100`                     | Maximum number of cached Slack clients (one per unique OAuth token). Prevents memory leaks in multi-user scenarios. Increase for high-traffic servers.                                                                                                                                    |
| `SLACK_MCP_CLIENT_CACHE_TTL`      | No        | `30m`                     | Time-to-live for cached Slack clients. Use Go duration format (e.g., `1h`, `45m`). Clients are evicted after this period to prevent stale connections.                                                                                                                                    |
| `SLACK_MCP_AUTH_CACHE_TTL`        | No        | `300` (5 minutes)         | Time-to-live (in seconds) for cached authentication validation results. Reduces redundant Slack API auth.test calls. Lower values increase security but reduce performance.                                                                                                               |
